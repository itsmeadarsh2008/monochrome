...
function cacheTrackMeta(id, title, artist, isrc, atmos) {
  if (!id || !title) return;
  TRACK_META_CACHE.set(String(id), {
    title,
...
// Cache TTL for a resolved stream response. Signed URLs die when their
// expiresAt passes, so the cache must never outlive the URL: leave a 2-minute
// flush margin and cap at 15 minutes (DASH MPDs live ~28min; /track/ URLs 6h
// but can be revoked early by TIDAL, so a conservative cap avoids poisoning
// clients with URLs that start 404ing).
function streamCacheTTL(stream) {
  const expiresAtSec = Number(stream?.expiresAt) || 0;
  const remaining = expiresAtSec - Math.floor(Date.now() / 1000);
  if (Number.isFinite(remaining) && remaining > 120) {
    return Math.max(60, Math.min(900, remaining - 120));
  }
  return 900;
}
...
function atmosPrefix(t) {
  if (!t) return "";
  const modes = Array.isArray(t.audioModes)
    ? t.audioModes
    : typeof t.audioMode === "string"
      ? [t.audioMode]
      : [];
...
function atmosSuffix(t) {
  ...
  const modes = Array.isArray(t.audioModes)
    ...
    : typeof t.audioMode === "string"
      ...
      : [];
  const tags = (t.mediaMetadata && t.mediaMetadata.tags) || [];
  if (modes.includes("SONY_360RA") || tags.includes("SONY_360RA"))
...
function withHardTimeout(promise, ms, label = "") {
  ...
  const guard = new Promise((_, reject) => {
    timer = setTimeout(
      () =>
        reject(
          new Error((label ? label + " " : "") + "timeout after " + ms + "ms"),
        ),
      ms,
    );
  });
...
function decodeManifest(manifest) {
  try {
    ...
    if (trimmed.startsWith("<")) {
      ...
      const cdnBase = raw.match(
        /https?:\/\/[a-z0-9\-\.]+\.tidal\.com\/[^\s"<>]+/i,
      );
      if (initAttr && cdnBase) {
        const init = initAttr[1]
    ...
    const url =
      decoded.urls && decoded.urls.length > 0
        ? decoded.urls[0]
        : decoded.url || null;
...
async function hifiGet(path, params) {
  const instances = instanceHealthy
    ? [activeInstance].concat(
        HIFI_INSTANCES.filter((i) => i !== activeInstance),
      )
...
async function hifiGetSafe(path, params) {
  try {
    return await hifiGet(path, params);
  } catch (e) {
    return null;
  }
}
...
async function hifiGetForToken(instanceUrl, path, params) {
  if (instanceUrl) {
    try {
      ...
      throw new Error(
        "Custom instance failed: " + instanceUrl + ": " + e.message,
      );
    }
  }
...
async function hifiGetForTokenSafe(instanceUrl, path, params) {
  try {
    return await hifiGetForToken(instanceUrl, path, params);
  } catch (e) {
    return null;
  }
}
...
// Workers modules format: secrets arrive on `env`, never as globals. They are
// captured at the top of every request/scheduled run and used lazily, so the
// rest of the code can keep calling upstashCmd() freely.
let UPSTASH_URL = null;
let UPSTASH_TOKEN = null;
function captureUpstashEnv(env) {
  if (!env) return;
  if (env.UPSTASH_REDIS_REST_URL) UPSTASH_URL = env.UPSTASH_REDIS_REST_URL;
...
const RATE_MAX = 120; // general requests per token per minute
const RATE_WINDOW_MS = 60000;

// Stream / download limits (per-token)
const STREAM_MAX_PER_MIN = 30; // max stream/download calls per token per minute
...
function checkStreamRateLimit(entry) {
  ...
  entry.streamWinMin = (entry.streamWinMin || []).filter(
    (t) => now - t < STREAM_WINDOW_MIN,
  );
  entry.streamWinHour = (entry.streamWinHour || []).filter(
    (t) => now - t < cooldown,
  );

  if (entry.streamWinMin.length >= STREAM_MAX_PER_MIN)
    return { ok: false, reason: "stream_min" };
  if (entry.streamWinHour.length >= STREAM_MAX_PER_HOUR) {
    // Escalate: 1hr → 2hr → 4hr max
    entry.streamCooldown = Math.min(
      (entry.streamCooldown || STREAM_WINDOW_HOUR) * 2,
      14400000,
    );
  ...
  if (!consumeGlobalStreamBudget())
    return { ok: false, reason: "global_stream" };
...
function checkBulkDownloadLimit(entry) {
  const now = Date.now();
  entry.bulkWin = (entry.bulkWin || []).filter(
    (t) => now - t < BULK_DL_WINDOW_MS,
  );
...
function checkSearchRateLimit(entry) {
  const now = Date.now();
  entry.searchWin = (entry.searchWin || []).filter(
...
async function getTokenEntry(token) {
  ...
  if (saved) {
    var entry = {
      ...
      preferredQuality: saved.preferredQuality || null,
      addonName: saved.addonName || null,
      rateWin: [],
    };
...
async function withToken(c, handler) {
  ...
  const { token, embeddedInstance, embeddedQuality } =
    parseTokenParam(rawParam);
...
function parseTokenParam(rawParam) {
  ...
  let embeddedInstance = null,
    embeddedName = null,
    embeddedQuality = null;
  try {
    if (parts[1])
      embeddedInstance = Buffer.from(parts[1], "base64url").toString("utf8");
  } catch (e) {}
  try {
    if (parts[2])
      embeddedName = decodeURIComponent(
        Buffer.from(parts[2], "base64url").toString("utf8"),
      ).slice(0, 40);
  } catch (e) {}
  try {
    if (parts[3])
      embeddedQuality = Buffer.from(parts[3], "base64url").toString("utf8");
  } catch (e) {}
...
function buildConfigPage(baseUrl) {
  ...
  h +=
    "h1{font-size:22px;font-weight:700;margin-bottom:6px;color:#fff}h2{font-size:16px;font-weight:700;margin-bottom:14px;color:#fff}";
  h += "p.sub{font-size:14px;color:#666;margin-bottom:20px;line-height:1.6}";
  h +=
    ".tip{background:#0a0a0a;border:1px solid #1e1e1e;border-radius:10px;padding:12px 14px;margin-bottom:20px;font-size:12px;color:#888;line-height:1.7}.tip b{color:#ccc}";
  ...
  h +=
    ".box{display:none;background:#0a0a0a;border:1px solid #1a1a1a;border-radius:12px;padding:18px;margin-bottom:14px}";
  h +=
  ...
  h +=
    ".steps{display:flex;flex-direction:column;gap:12px}.step{display:flex;gap:12px;align-items:flex-start}";
  h +=
  ...
  h +=
    ".ql-group-label{font-size:10px;color:#555;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;margin-top:12px}";
  h += ".ql-row{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:4px}";
  h +=
    ".ql-btn{flex:1;min-width:calc(50% - 4px);cursor:pointer;border:1px solid #2a2a2a;border-radius:12px;background:#0a0a0a;color:#555;font-size:12px;font-weight:700;padding:12px 8px;text-align:center;transition:all .15s;letter-spacing:.04em;line-height:1.4}";
  ...
  h +=
    ".ql-sub{font-size:10px;font-weight:400;opacity:.6;display:block;margin-top:2px}";
  h +=
    ".qual-badge{display:inline-flex;align-items:center;gap:6px;padding:6px 12px;border-radius:20px;font-size:12px;font-weight:600;margin-bottom:10px}";
  h +=
    ".qual-badge.tidal{background:#120d20;color:#9b4aff;border:1px solid #1a2a40}";
  ...
  h +=
    '.inst-url{flex:1;color:#666;font-family:"SF Mono","Fira Code",monospace;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}';
  h += ".inst-ms{color:#444;margin-left:auto;font-size:11px}";
  h += "</style></head><body>";

  // Logo
  h +=
    '<svg width="52" height="52" viewBox="0 0 52 52" fill="none" style="margin-bottom:22px"><circle cx="26" cy="26" r="26" fill="#fff"/><rect x="10" y="20" width="4" height="12" rx="2" fill="#000"/><rect x="17" y="14" width="4" height="24" rx="2" fill="#000"/><rect x="24" y="18" width="4" height="16" rx="2" fill="#000"/><rect x="31" y="11" width="4" height="30" rx="2" fill="#000"/><rect x="38" y="17" width="4" height="18" rx="2" fill="#000"/></svg>';

  h += '<div class="card">';
  h += "<h1>Claudo for Eclipse</h1>";
  h +=
    '<p class="sub">Full TIDAL catalog &mdash; FLAC Hi-Res 24-bit up to 192kHz DASH, FLAC 16-bit, AAC fallback &mdash; no account needed.</p>';
  h +=
    '<div class="tip"><b>Save your URL.</b> Paste it below to refresh without reinstalling.</div>';
  h +=
    '<div class="pills"><span class="pill">Tracks &middot; Albums &middot; Artists</span><span class="pill hi">FLAC Hi-Res 24-bit</span><span class="pill hi">FLAC 16-bit</span><span class="pill hi">Dolby Atmos</span></div>';

  h +=
    '<div class="lbl">Custom Hi&#8209;Fi Instance <span style="color:#2a2a2a;font-weight:400;text-transform:none">(optional)</span></div>';
  h +=
    '<input type="text" id="customInstance" placeholder="https://your-instance.example.com">';
  h +=
    '<div class="hint">Leave blank to use the shared pool. Paste your own self-hosted Hi-Fi API URL to lock this token exclusively to your instance.</div>';

  // Quality selector — TIDAL tiers, single selection
  h +=
    '<div class="lbl">Preferred Audio Quality <span style="color:#2a2a2a;font-weight:400;text-transform:none">(optional)</span></div>';

  h += '<div class="ql-group-label">&#9679; TIDAL</div>';
  h += '<div class="ql-row">';
  h +=
    '<div class="ql-btn" id="ql-HIMAX"    onclick="selectQ(\'HIMAX\',\'q\')">Hi-Res 192<span class="ql-sub">24-bit / up to 192kHz</span></div>';
  h +=
    '<div class="ql-btn" id="ql-HI96"     onclick="selectQ(\'HI96\',\'q\')">Hi-Res 96<span class="ql-sub">24-bit / up to 96kHz</span></div>';
  h += "</div>";
  h += '<div class="ql-row">';
  h +=
    '<div class="ql-btn" id="ql-LOSSLESS" onclick="selectQ(\'LOSSLESS\',\'q\')">CD Quality<span class="ql-sub">16-bit / 44.1kHz</span></div>';
  h +=
    '<div class="ql-btn" id="ql-AAC320"   onclick="selectQ(\'AAC320\',\'q\')">AAC 320<span class="ql-sub">320 kbps AAC</span></div>';
  h += "</div>";

  h +=
    '<div class="hint" id="qlHint" style="margin-top:8px">No preference &mdash; auto-selects best available quality: Hi-Res 192kHz &rarr; Hi-Res 96kHz &rarr; CD Quality &rarr; AAC 320.</div>';

  h +=
    '<div class="lbl">Addon Name <span style="color:#2a2a2a;font-weight:400;text-transform:none">(optional)</span></div>';
  h +=
    '<input type="text" id="customAddonName" placeholder="Claudo" maxlength="40">';
  h +=
    '<div class="hint">Customize the name shown in Eclipse\'s connections list. Leave blank to keep the previous name.</div>';
  h +=
  ...
  h +=
    '<div class="hint">Keeps the same URL active &mdash; nothing to reinstall.</div>';
  h +=
    '<button class="bg" id="refBtn" onclick="doRefresh()">Refresh Existing URL</button>';
  h +=
    '<div class="box" id="refBox"><div class="blbl">Refreshed &mdash; same URL still works in Eclipse</div><div class="burl" id="refUrl"></div><button class="bd" id="copyRefBtn" onclick="copyRef()">Copy URL</button></div>';

  h += "<hr>";
  h += '<div class="steps">';
  h +=
    '<div class="step"><div class="sn">1</div><div class="st">Select a quality tier above, then click <b>Generate</b></div></div>';
  h +=
  ...
  h +=
    '<div class="step"><div class="sn">4</div><div class="st">Search TIDAL\'s full catalog &mdash; Hi-Res FLAC DASH streams play first automatically</div></div>';
  h += "</div>";
  h +=
    '<div class="warn">Stream priority: <b>Hi-Res FLAC 24-bit</b> (DASH, up to 192kHz) &rarr; CD Quality FLAC 16-bit &rarr; AAC 320. Dolby Atmos tracks stream Atmos-only when available.</div>';
  h += "</div>";
  ...
  h +=
    '<p class="sub" style="margin-bottom:14px">Live status of all Hi-Fi API instances.</p>';
  h +=
  ...
  h +=
    '<button class="bd" style="width:auto;padding:6px 14px;font-size:12px;margin:0" onclick="runQualityTest()">Refresh</button>';
  h += "</div>";
  h +=
    '<p class="sub" style="margin-bottom:14px">Tests if each audio quality tier is reachable on the active HiFi instance.</p>';
  h +=
    '<div class="inst-list" id="qtList"><div style="color:#333;font-size:13px">Checking tiers...</div></div>';
  h += "</div>";

  h +=
    "<footer>Claudo Eclipse Addon &bull; TIDAL search &bull; Hi-Res FLAC DASH streams</footer>";
  ...
  h += "  if(selQ){";
  h +=
    '    hint.textContent="Selected: "+QLABELS[selQ]+" \u2014 Hi-Res FLAC DASH first.";';
  h += "  }else{";
  h +=
    '    hint.textContent="No preference \u2014 auto-selects best quality: Hi-Res \u2192 CD Quality \u2192 AAC 320.";';
  h += "  }";
  h += "  updateBadge();";
  h += "}";
  ...
  h += "  if(!b)return;";
  h +=
    '  if(!selQ){b.className="qual-badge none";b.textContent="No quality preference";return;}';
  h +=
    '  b.className="qual-badge tidal";b.innerHTML="&#9679; TIDAL \u00b7 "+QLABELS[selQ];';
  h += "}";
  ...
  h += "    updateBadge();";
  h +=
    '    document.getElementById("genQualBadge").style.display="inline-flex";';
  ...
  h +=
    'function copyGen(){if(!gu)return;try{navigator.clipboard.writeText(gu);}catch(e){var t=document.createElement("textarea");t.value=gu;document.body.appendChild(t);t.select();document.execCommand("copy");document.body.removeChild(t);}var b=document.getElementById("copyGenBtn");b.textContent="Copied!";setTimeout(function(){b.textContent="Copy URL";},1500);}';
  h +=
    'function doRefresh(){var btn=document.getElementById("refBtn");var eu=document.getElementById("existingUrl").value.trim();if(!eu){alert("Paste your existing addon URL first.");return;}btn.disabled=true;btn.textContent="Refreshing...";fetch("/refresh",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({existingUrl:eu})}).then(function(r){return r.json();}).then(function(d){if(d.error){alert(d.error);btn.disabled=false;btn.textContent="Refresh Existing URL";return;}ru=d.manifestUrl;document.getElementById("refUrl").textContent=ru;document.getElementById("refBox").style.display="block";btn.disabled=false;btn.textContent="Refresh Again";}).catch(function(e){alert("Error: "+e.message);btn.disabled=false;btn.textContent="Refresh Existing URL";});}';
  h +=
    'function copyRef(){if(!ru)return;try{navigator.clipboard.writeText(ru);}catch(e){var t=document.createElement("textarea");t.value=ru;document.body.appendChild(t);t.select();document.execCommand("copy");document.body.removeChild(t);}var b=document.getElementById("copyRefBtn");b.textContent="Copied!";setTimeout(function(){b.textContent="Copy URL";},1500);}';
  ...
  h +=
    '  fetch("/instances").then(function(r){return r.json();}).then(function(data){';
  h += '    list.innerHTML="";';
  h += "    data.instances.forEach(function(inst){";
  h += '      var row=document.createElement("div");row.className="inst";';
  h +=
    '      var dot=document.createElement("span");dot.className="dot "+(inst.ok?"ok":"err");';
  h +=
    '      var urlSpan=document.createElement("span");urlSpan.className="inst-url";';
  h += "      urlSpan.textContent=inst.url.split('-').pop().split('.')[0];";
  h += "      row.appendChild(dot);row.appendChild(urlSpan);";
  h +=
    '      if(inst.ok){var ms=document.createElement("span");ms.className="inst-ms";ms.textContent=inst.ms+"ms";row.appendChild(ms);}';
  h += "      list.appendChild(row);";
  h += "    });";
  h +=
    "  }).catch(function(){list.innerHTML='<div style=\"color:#c04040;font-size:13px\">Could not reach server</div>';});";
  h += "}";

  h += "function runQualityTest(){";
  h += 'var list=document.getElementById("qtList");';
  h +=
    "list.innerHTML='<div style=\"color:#333;font-size:13px\">Checking tiers...</div>';";
  h +=
    'fetch("/quality-test").then(function(r){return r.json();}).then(function(d){';
  h += 'list.innerHTML="";';
  h += "(d.results||[]).forEach(function(t){";
  h += 'var row=document.createElement("div");row.className="inst";';
  h +=
    'var dot=document.createElement("span");dot.className="dot "+(t.ok?"ok":"err");';
  h +=
    'var label=document.createElement("span");label.className="inst-url";label.textContent=t.label;';
  h += "row.appendChild(dot);row.appendChild(label);";
  h +=
    'if(t.ok){var ms=document.createElement("span");ms.className="inst-ms";ms.textContent=t.ms+"ms";row.appendChild(ms);}';
  h += "list.appendChild(row);";
  h += "});";
  h +=
    'var s=d.summary||{};var sum=document.createElement("div");sum.style.cssText="margin-top:10px;font-size:12px;color:#555";';
  h += 'sum.textContent="TIDAL: "+s.tidal;';
  h +=
    'if(s.atmos){var a=document.createElement("div");a.style.cssText="margin-top:2px;font-size:12px;color:"+(s.atmos.indexOf("OK")===0?"#4a9a4a":"#c04040");a.textContent="Dolby Atmos: "+s.atmos;sum.appendChild(a);}';
  h += "list.appendChild(sum);";
  h +=
    "}).catch(function(e){list.innerHTML='<div style=\"color:#c04040;font-size:13px\">Test failed: '+e.message+'</div>';});";
  h += "}";
...
app.get("/", async (c) => {
  const baseUrl =
    ...
    "://" +
    c.req.header("host");
  return new Response(buildConfigPage(baseUrl), {
...
app.post("/generate", async (c) => {
  ...
  if (bucket.count >= MAX_TOKENS_PER_IP)
    return Response.json(
      {
        error:
          "Too many tokens from this IP today. Max " +
          MAX_TOKENS_PER_IP +
          " per day.",
      },
      { status: 429 },
  ...
  const genBucket = IP_GEN_RATE.get(ip) || {
    count: 0,
    resetAt: Date.now() + 3600000,
  };
  if (Date.now() > genBucket.resetAt) {
    genBucket.count = 0;
    genBucket.resetAt = Date.now() + 3600000;
  }
  if (genBucket.count >= MAX_GEN_PER_HOUR)
    return Response.json(
      { error: "Too many tokens generated this hour. Try again in an hour." },
      { status: 429 },
    );
  genBucket.count++;
  IP_GEN_RATE.set(ip, genBucket);
  let instanceUrl =
    body && body.instanceUrl
      ? String(body.instanceUrl).trim().replace(/\/$/, "")
      : null;
  if (instanceUrl) {
    if (!/^https?:\/\//.test(instanceUrl))
      return Response.json(
        { error: "Instance URL must start with http or https" },
        { status: 400 },
      );
    try {
      await axios.get(instanceUrl + "/search/", {
        params: { s: "test", limit: 1 },
        timeout: 8000,
      });
    } catch (e) {
      return Response.json(
        { error: "Could not reach your instance: " + e.message },
        { status: 400 },
      );
    }
  }
  ...
  const preferredQuality =
    ...
    VALID_QUALITIES.includes(body.preferredQuality)
      ? body.preferredQuality
      : null;
  const token = generateToken();
  const addonName =
    body && body.addonName && String(body.addonName).trim()
      ? String(body.addonName).trim().slice(0, 40)
      : null;
  const entry = {
    createdAt: Date.now(),
    lastUsed: Date.now(),
  ...
  const baseUrl =
    ...
    "://" +
    c.req.header("host");
  let tokenSegment = instanceUrl
    ? token + "~" + Buffer.from(instanceUrl).toString("base64url")
    : token;
  if (addonName) {
    if (!instanceUrl) tokenSegment += "~";
    tokenSegment +=
      "~" + Buffer.from(encodeURIComponent(addonName)).toString("base64url");
  }
  if (preferredQuality) {
...
app.post("/refresh", async (c) => {
  ...
  if (!token || !/^[a-f0-9]{28}$/.test(token))
    return Response.json(
      { error: "Paste your full addon URL." },
      { status: 400 },
    );
  const entry = await getTokenEntry(token);
  if (!entry)
    return Response.json(
      { error: "URL not found. Generate a new one." },
      { status: 404 },
    );
  const baseUrl =
    (c.req.header("x-forwarded-proto") || "https") +
...
app.get("/quality-test", async (c) => {
  ...
  const TIERS = [
    ...
    {
      id: "tidal_hires",
      label: "TIDAL Hi-Res FLAC",
      type: "tidal",
      quality: "HI_RES_LOSSLESS",
    },
    {
      id: "tidal_lossless",
      label: "TIDAL FLAC 16-bit",
      type: "tidal",
      quality: "LOSSLESS",
    },
    {
    ...
    TIERS.map(async (tier) => {
      const attempt = async () => {
        ...
        try {
          ...
          if (tier.quality === "HI_RES_LOSSLESS" || tier.quality === "LOSSLESS")
            params.manifest = "hls";
          ...
          if (payload && payload.manifest)
            dec = decodeManifest(payload.manifest);
  ...
  return Response.json({
    ...
    rateLimits: {
      ...
      globalDayResetIn:
        Math.max(
          0,
          Math.ceil((86400000 - (Date.now() - globalDayStart)) / 60000),
        ) + "min",
    },
...
app.get("/u/:token/manifest.json", async (c) => {
  return withToken(c, (entry) => {
    ...
    return Response.json({
      ...
      name: (() => {
        const { embeddedName } = parseTokenParam(c.req.param("token"));
        return embeddedName || entry.addonName || "Claudo";
      })(),
      version: "3.0.0",
      description:
        "TIDAL catalog search + Hi-Res 24-bit FLAC DASH streams. FLAC/Lossless/AAC. No account required.",
...
app.get("/u/:token/search", async (c) => {
  return withToken(c, async (entry) => {
    const q = String(
      c.req.query("q") || c.req.query("query") || c.req.query("s") || "",
    ).trim();
    ...
    const cacheKey =
      "mc:search:" + (inst || "pool") + ":" + q.toLowerCase() + ":" + limit;
    ...
    if (redisCached) {
      ...
      if (redisCached.tracks)
        redisCached.tracks.forEach((t) => {
          if (t && t.id && t.title && !TRACK_META_CACHE.has(String(t.id)))
            cacheTrackMeta(t.id, t.title, t.artist);
        });
      // Stale-while-revalidate: rebuild in the background only when the entry is
      // older than 6h (TTL left < 18h of the 24h lifetime). The refresh build is
      // spawned here (not at request time) so cache hits stay budget-free — the
      // miss path below kicks off its own build after the rate check.
      upstashCmd("TTL", cacheKey)
        .then((remaining) => {
          if (
            ...
            buildSearchResult(q, limit, inst, Promise.resolve(true))
              .then((r) => {
                if (r) {
                  cSet(cacheKey, r, 300);
    ...
    // Cached queries never touch upstream, so they must not consume the search
    // budget — the limiter only gates the expensive cache-miss path that will
    // actually build (and hit TIDAL).
    if (!checkSearchRateLimit(entry)) {
      return Response.json(
        {
          error:
            "Search rate limit exceeded. Max " +
            SEARCH_MAX_PER_MIN +
            " searches per minute.",
          tracks: [],
          albums: [],
    ...
    const buildPromise = buildSearchResult(
      q,
      limit,
      inst,
  ...
  async function buildSearchResult(q, limit, inst, compositeMissP) {
    ...
    const subFetch = async (key, fn, timeoutMs) => {
      const mem = cGet(key);
      if (mem) return mem;
      const skipSubCache = compositeMissP
        ? await compositeMissP.catch(() => false)
        : false;
      if (!skipSubCache) {
      ...
      if (val) {
        cSet(key, val, 600);
        upstashCmd("SET", key, JSON.stringify(val), "EX", 86400).catch(
          () => {},
        );
      }
      return val;
    ...
    const [mainData, alData, arData, plData] = await Promise.all([
      ...
      subFetch(
        sKey("al", Math.min(limit, 20)),
        () =>
          hifiGetForTokenSafe(inst, "/search/", {
            al: q,
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (t.album && t.album.id) {
        ...
        if (!albumMap[abid])
          albumMap[abid] = {
            ...
            artworkURL: coverUrl(t.album.cover, 1080),
            trackCount: t.album.numberOfTracks,
            year: t.album.releaseDate
              ? String(t.album.releaseDate).slice(0, 4)
              : undefined,
          };
      }
        ...
        if (!artistMap[arid])
          artistMap[arid] = {
            ...
            ...(a.genres && a.genres.length
              ? { genres: a.genres.map((g) => g.name || g).filter(Boolean) }
              : {}),
          };
      ...
      const tFormat =
        t.audioQuality === "HIGH" || t.audioQuality === "LOW" ? "aac" : "flac";
      tracks.push({
        id: String(t.id),
        title: atmosPrefix(t) + tTitle + atmosSuffix(t),
        artist: tArtist,
        album: t.album ? t.album.title : undefined,
        albumId: t.album?.id ? String(t.album.id) : undefined,
        albumArtworkURL: t.album?.cover
          ? coverUrl(t.album.cover, 1080)
          : undefined,
        ...
        artworkURL: coverUrl(t.album ? t.album.cover : null, 1080),
        format: tFormat,
        isrc: t.isrc || undefined,
        audioQuality: t.audioQuality || undefined,
        provider: "Tidal",
        audioModes: t.audioModes || undefined,
      });
    }
    ...
    const artistList = Object.keys(artistMap)
      .sort(
        (a, b) =>
          artistRelevance(artistMap[b].name, q) * 100 +
          (artistHits[b] || 0) -
          (artistRelevance(artistMap[a].name, q) * 100 + (artistHits[a] || 0)),
      )
      .slice(0, 5)
      .map((k) => artistMap[k]);
    ...
    const plFromSearch =
      plData?.data?.playlists?.items ||
      plData?.data?.playlists ||
    ...
    const plItems = [];
    for (const p of [
      ...(Array.isArray(plFromSearch) ? plFromSearch : []),
      ...
      plItems.push({
        ...
        creator:
          p.creator?.name || (p.type === "EDITORIAL" ? "TIDAL" : undefined),
    ...
    for (const t of tracks) {
      const key =
        (t.title || "").toLowerCase().replace(/[^a-z0-9]/g, "") +
        "||" +
...
async function getTidalDashStream(tid, inst, atmosOnly) {
  ...
  if (atmosOnly && cls !== 3) {
    console.log(
      "[tidal] Atmos track " +
        tid +
        ": /dash/ returned non-Atmos MPD — no fallback to stereo",
    );
    return null;
  }
...
app.get("/u/:token/stream/:id", async (c) => {
  return withToken(c, async (entry) => {
    ...
    const atmosDedupeKey = String(c.req.query("title") || "")
      .trim()
      .startsWith("◗◖ ")
      ? ":atmos"
      : "";
    ...
    // ── Cache-first: repeated plays of the same track must not re-resolve ─────
    // A fresh resolution costs a stream-budget token, a TIDAL API call and an
    // instance /dash/ fetch every time. Signed URLs stay valid for minutes-to-
    // hours, so a cached hit serves the same URL without touching any upstream.
    // Cache hits are exempt from the rate limiter — no upstream work was done.
    const memStream = cGet(streamCacheKey);
    if (memStream) {
      console.log("[stream] cache HIT " + streamCacheKey);
      return Response.json(memStream);
    }
    const redisStream = await upstashCmd("GET", "mc:" + streamCacheKey)
      .then((raw) => {
        if (!raw) return null;
        try {
          const p = JSON.parse(raw);
      ...
      .catch(() => null);
    if (redisStream) {
      console.log("[stream] redis HIT " + streamCacheKey);
      cSet(streamCacheKey, redisStream, streamCacheTTL(redisStream));
      return Response.json(redisStream);
    }
    ...
    if (!streamCheck.ok) {
      const msgs = {
        stream_min:
          "Stream rate limit: too fast. Max " +
          STREAM_MAX_PER_MIN +
          " per minute.",
        stream_hour:
          "Hourly stream limit reached (" +
          STREAM_MAX_PER_HOUR +
          " tracks). Resume in " +
          Math.ceil((entry.streamCooldown || STREAM_WINDOW_HOUR) / 3600000) +
          " hour(s).",
        global_stream:
          "Service stream capacity reached for today. Try again tomorrow.",
      };
      return Response.json(
        { error: msgs[streamCheck.reason] || "Stream rate limit exceeded." },
        { status: 429 },
      );
    }
    ...
    if (!checkBulkDownloadLimit(entry)) {
      return Response.json(
        {
          error:
            "Download limit reached. Max " +
            BULK_DL_MAX +
            " tracks per 10 minutes. Wait before resuming. Large playlist downloads are not supported.",
        },
        { status: 429 },
      );
    }
      ...
      async () => {
        ...
        if (!qTitle) {
          ...
          if (mem) {
            qTitle = mem.title;
            qArtist = mem.artist;
            if (!qIsrc) qIsrc = mem.isrc || null;
            console.log(
              "meta: hit in-memory cache for",
              tid,
              "->",
              qTitle,
              qIsrc ? "(isrc: " + qIsrc + ")" : "",
        ...
        if (redisMeta) {
          if (!qTitle) {
            ...
            if (!qIsrc) qIsrc = redisMeta.isrc || null;
            console.log(
              "meta: hit Redis cache for",
              tid,
              "->",
              qTitle,
              qIsrc ? "(isrc: " + qIsrc + ")" : "",
            ...
            qTitle = redisMeta.title;
            qArtist = redisMeta.artist;
            if (redisMeta.isrc) qIsrc = redisMeta.isrc;
          }
        }
        ...
        if (!atmosOnly && !atmosKnown) {
          try {
            const trackInfo = await hifiGetForTokenSafe(inst, "/info/", {
              id: tid,
            });
            ...
            if (payload && payload.title) {
              ...
              cacheTrackMeta(
                tid,
                payload.title,
                trackArtist(payload) || qArtist,
                payload?.isrc || qIsrc || null,
                vAtmos,
              );
              redisCacheTrackMeta(
                String(tid),
              ...
              console.log(
                "meta: /info/ verify for",
                tid,
                "->",
                vAtmos ? "DOLBY_ATMOS (no fallback)" : "not Atmos",
              );
            }
          } catch (e) {
        ...
        if (atmosOnly)
          console.log(
            "meta: track",
            tid,
            "is DOLBY_ATMOS — Atmos-only streaming, no fallback",
          );
        ...
        if (!qTitle && !qIsrc) {
          try {
            ...
            if (coldTitle) {
              ...
              console.log(
                "meta: HiFi cold-lookup HIT for tid",
                tid,
                "->",
                qTitle,
                qIsrc ? "(isrc: " + qIsrc + ")" : "",
                coldAtmos ? "(DOLBY_ATMOS)" : "",
              );
            ...
            console.log(
              "meta: HiFi cold-lookup failed for tid",
              tid,
              "-",
              e.message,
            );
        ...
        const PREF_TO_TIDAL = {
          ...
          HIMAX: "HI_RES_LOSSLESS",
          HI96: "HI_RES_LOSSLESS",
          LOSSLESS: "LOSSLESS",
          HIGH: "HIGH",
          AAC320: "HIGH",
          LOW: "LOW",
          ...
          TIDAL_HIMAX: "HI_RES_LOSSLESS",
          TIDAL_LOSSLESS: "LOSSLESS",
          TIDAL_HIGH: "HIGH",
          TIDAL_LOW: "LOW",
        };
        ...
        async function getTidalStream() {
          ...
          const tidalStartTier = pref
            ? PREF_TO_TIDAL[pref] || "LOSSLESS"
            : null;
          const qualities = atmosOnly
            ? ["HI_RES_LOSSLESS"]
            : tidalStartTier
              ? [
                  tidalStartTier,
                  ...ALL_QUALITIES.filter(
                    (q) =>
                      ALL_QUALITIES.indexOf(q) >
                      ALL_QUALITIES.indexOf(tidalStartTier),
                  ),
                ]
              : AUTO_QUALITIES;
            ...
            try {
              ...
              if (payload && payload.manifest) {
                ...
                if (decoded && decoded.url) {
                  ...
                  if (atmosOnly) {
                    ...
                    if (!codec.includes("ec-3") && !codec.includes("ac-4")) {
                      console.log(
                        "[tidal] Atmos track " +
                          tid +
                          ' returned non-Atmos codec "' +
                          codec +
                          '" at ' +
                          ql +
                          " — no fallback to stereo",
                      );
                      continue;
                    }
                    console.log(
                    ...
                    return {
                      url: decoded.url,
                      format: "aac",
                      quality: "Tidal · Dolby Atmos",
                      streamQuality: "[Tidal] DOLBY_ATMOS",
                  ...
                  const isExplicitFlac =
                    codec.includes("flac") || codec.includes("audio/flac");
                  const isExplicitAac =
                    codec.includes("mp4a") ||
                    codec.includes("aac") ||
                    codec.includes("he-aac");
                  const isLosslessTier =
                  ...
                  const isFlac =
                    isExplicitFlac || (!isExplicitAac && isLosslessTier);
                  const qualityLabel =
                    ql === "HI_RES_LOSSLESS"
                      ? "Hi-Res FLAC"
                      : ql === "LOSSLESS"
              ...
              if (payload && payload.url) {
                if (atmosOnly) {
                  ...
                  console.log(
                    "[tidal] Atmos track " +
                      tid +
                      " got direct-URL payload without Atmos codec — no fallback to stereo",
                  );
                  continue;
                }
                ...
                return {
                  url: payload.url,
                  format: looksLikeFlac || isLosslessTier ? "flac" : "aac",
                  quality: "Tidal · " + qualityLabel,
                  streamQuality: "[Tidal] " + streamQuality,
                  expiresAt: Math.floor(Date.now() / 1000) + 21600,
                };
        ...
        if (best) {
          const ttlSec = streamCacheTTL(best);
          cSet(streamCacheKey, best, ttlSec);
          // The Redis write must survive the request: keep the isolate alive
          // until Upstash acknowledges, otherwise the fire-and-forget fetch can
        ...
        if (atmosOnly) {
          console.log(
            "[stream] Atmos track " +
              tid +
              " has no Atmos source — refusing stereo fallback",
          );
          return Response.json(
            {
              error:
                "Dolby Atmos is unavailable for this track. The configured TIDAL instance does not serve Atmos audio, and no fallback is applied to Atmos tracks.",
              atmos: true,
            },
            { status: 502 },
          );
        ...
        return Response.json(
          { error: "No stream found for track " + tid },
          { status: 404 },
        );
      },
    ); // end dedupeCall
...
app.get("/u/:token/album/:id", async (c) => {
  return withToken(c, async (entry) => {
    ...
    const inst = entry.instanceUrl;
    try {
      const data = await hifiGetForToken(inst, "/album/", {
        id: aid,
        limit: 100,
        offset: 0,
      });
      // Unwrap all known HiFi API response shapes
      const album = data?.data?.id
        ? data.data
        : data?.data?.album?.id
          ? data.data.album
          : data?.album?.id
      ...
      const tracks = rawItems
        .map((item, i) => {
          ...
          const tFormat =
            t.audioQuality === "HIGH" || t.audioQuality === "LOW"
              ? "aac"
              : "flac";
          return {
            id: String(t.id),
            ...
            artworkURL: coverUrl(cover, 1080),
            format: tFormat,
            isrc: t.isrc || undefined,
          };
        })
      ...
      return Response.json(
        { error: "Album fetch failed: " + e.message },
        { status: 502 },
      );
    }
  });
  ...
  return withToken(c, async (entry) => {
    ...
    try {
      ...
      const [
        ...
        axios.get(base + "/artist/", {
          params: { id: aid },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/", {
          params: { f: aid, skip_tracks: false },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/discography/", {
          params: { id: aid, limit: 100 },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/toptracks/", {
          params: { id: aid, limit: 30 },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/albums/", {
          params: { id: aid, limit: 100, offset: 0 },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/albums/", {
          params: { id: aid, filter: "ALBUMS", limit: 100 },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/albums/", {
          params: { id: aid, filter: "EPSSINGLES", limit: 100 },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/albums/", {
          params: { id: aid, filter: "COMPILATIONS", limit: 100 },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
        axios.get(base + "/artist/albums/", {
          params: { artistId: aid, limit: 100 },
          headers: { "User-Agent": UA, Accept: "application/json" },
          timeout: 12000,
        }),
      ...
      const topTracks = Object.values(trackMap)
        ...
        .map((t) => {
          ...
          return {
            ...
            duration: trackDuration(t),
            artworkURL: coverUrl(
              t.album?.cover || t.album?.image || t.album?.artwork,
              1080,
            ),
          };
        });
      ...
      const albums = Object.values(albumMap)
        .sort((a, b) => {
          const ya = a.releaseDate
            ? parseInt(String(a.releaseDate).slice(0, 4), 10)
            : 0;
          const yb = b.releaseDate
            ? parseInt(String(b.releaseDate).slice(0, 4), 10)
            : 0;
      ...
      const artistGenres =
        artistInfo.genres && artistInfo.genres.length
      ...
      return Response.json(
        { error: "Artist fetch failed: " + e.message },
        { status: 502 },
      );
    }
  });
...
app.get("/u/:token/playlist/:id", async (c) => {
  return withToken(c, async (entry) => {
    ...
    if (!isPlaylistUUID(pid))
      return Response.json(
        { error: "Invalid playlist ID. TIDAL playlist IDs must be UUIDs." },
        { status: 404 },
      );
    try {
      const data = await hifiGetForToken(inst, "/playlist/", {
        id: pid,
        limit: 100,
        offset: 0,
      });
      ...
      return Response.json({
        ...
        artworkURL:
          pl?.squareImage || pl?.image
            ? coverUrl(pl.squareImage || pl.image, 1080)
            : undefined,
      ...
      return Response.json(
        { error: "Playlist fetch failed: " + e.message },
        { status: 502 },
      );
    }
  });
...
app.get("/8spine", async (c) => {
  ...
  return c.json({
    ...
    version: "3.0.0",
    description:
      "TIDAL full catalog search + Hi-Res 24-bit FLAC DASH streams. FLAC/Lossless/HiRes/Atmos. No account required.",
...
// ─── Diagnostics: Redis/Upstash health ───────────────────────────────────────
app.get("/__diag", async () => {
  const ping = await upstashCmd("PING");
  return Response.json({
    upstashUrlConfigured: !!UPSTASH_URL,
    upstashTokenConfigured: !!UPSTASH_TOKEN,
    ping,
    memoryCacheSize: _cache.size,
  });
});
...
async function prewarmPopular() {
  try {
    const queries = await upstashCmd(
      "ZREVRANGE",
      "mc:popular:top",
      0,
      PREWARM_TOP_N - 1,
    ...
    console.log(
      "[prewarm] refreshing " + queries.length + " popular search(es)",
    );
    for (let i = 0; i < queries.length; i += 5) {
      await Promise.all(
        queries.slice(i, i + 5).map(async (q) => {
